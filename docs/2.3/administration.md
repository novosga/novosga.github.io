...
Cadastro das unidades de atendimento do sistema. Unidade de atendimento é o local físico aonde os atendimentos são executados. Por exemplo: Uma unidade de saúde, um departamento de atendimento ao cliente, ou uma localidade.

A partir da versão 2.3, cada unidade possui um campo de **fuso horário** independente, permitindo que unidades em diferentes regiões utilizem o horário local correto para geração e exibição de senhas.
...
A partir da versão 2.3, é possível configurar por lotação quais **tipos de senha** o atendente pode emitir: somente normal, somente prioritária, somente agendamento, ou qualquer combinação.
...
Cadastro dos painéis de atendimento integrados ao sistema. A partir da versão 2.3, o NovoSGA inclui um módulo de painel nativo que pode ser acessado publicamente, sem autenticação, através de uma URL única gerada automaticamente.

Cada painel pode ser configurado para exibir chamadas de serviços específicos de uma determinada unidade.
